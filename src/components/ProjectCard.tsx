...
export default function ProjectCard({ project } : { project: repository }) {
    ...
    return (
        <Link href={project.url} target="_blank" className="hover:-translate-y-5 transition-all duration-150">
            <div className="w-full border px-5 py-2 min-h-64 max-md:min-h-56 min-w-fit rounded-xl mb-2 relative">
                <div className="mb-2 pr-40 max-md:pr-30 relative">
                    <h3 className="text-3xl max-md:text-2xl">{project.name}</h3> 
                    <span className="opacity-70 max-md:text-sm">
                        {project.description ? (project.description).slice(0, 45) + "..." : ''}
                    </span>    
                    <div className="flex gap-5 absolute right-0 top-2 max-md:top-1">
                ...
                <div className="flex justify-between gap-5 max-md:text-md">
                    <span>
                ...
                <div className="flex gap-2 items-baseline max-md:text-sm">        
                    <span>
                        {project.defaultBranchRef.target.history.totalCount} Commits
                    </span>

                    <span className="text-sm max-md:text-xs opacity-70">
                        {(() => {
                    ...
                    <div className="flex flex-wrap gap-x-4 w-full h-full mt-2">
                        {project.languages.edges.map((language, idx) => (
                            <div key={idx} className="text-md flex items-center mr-4">
                                <div 
                                    className="mr-2 w-2 h-2 max-md:text-sm rounded-full" 
                                    style={{ backgroundColor: languageColors[language.node.name] ? languageColors[language.node.name] : "#333"}}>
                                </div>
                                {language.node.name}
                                <span className="text-sm max-md:text-xs text-gray-400 ml-1">
